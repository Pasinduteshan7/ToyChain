...
//
//	RECORD: Time taken is measured with time.Since() for reporting.
func (bc *Blockchain) MineBlock(txs []ledger.Transaction) (MiningResult, error) {
	// 1. PRE-MINING VALIDATION: Ensure the transactions are legal before we waste
	// CPU cycles mining them. Without this, MineBlock could be used as a backdoor
	// to inject invalid transactions directly into the chain.
	projected := bc.Balances()
	for _, tx := range txs {
		if err := projected.Apply(tx); err != nil {
			return MiningResult{}, fmt.Errorf("transaction rejected by ledger rules: %v", err)
		}
	}
	...
	// THE CONCURRENT PROOF-OF-WORK LOOP (FR-5 Stretch Goal):
	// Instead of searching one nonce at a time on a single core, we
	// spawn a goroutine for each CPU core. If we have 8 cores, Worker 1
	// checks nonces 0, 8, 16... Worker 2 checks 1, 9, 17... and so on.
	// The first worker to find a valid hash wins, and we cancel the rest.
	...
	for i := 0; i < numWorkers; i++ {
		go func(workerIndex int) {
			// Each worker needs its own local block copy to safely hash in parallel
			localBlock := block.New(newHeight, b.Timestamp, txs, tip.Hash)
			localBlock.Nonce = workerIndex
			attempts := 0

			for {
				// Check if another worker already won
				select {
				case <-ctx.Done():
					return
				default:
				...
				if strings.HasPrefix(localBlock.Hash, target) {
					// We found a valid hash! Try to send it back.
					select {
					case resultChan <- workResult{localBlock.Nonce, localBlock.Hash, attempts}:
					case <-ctx.Done(): // In case another worker won just a microsecond earlier
					}
					return
				}
				
				// Jump ahead by the number of workers to avoid checking the same nonces
				localBlock.Nonce += numWorkers
			}
		}(i)
	}

	// Wait for the first worker to succeed
	winner := <-resultChan
	cancel() // Tell all other workers to stop searching

	// Apply the winning results to our actual block
	b.Nonce = winner.nonce
	b.Hash = winner.hash
	
	// Approximate total attempts (winner's attempts * number of concurrent workers)
	attempts := winner.attempts * numWorkers
